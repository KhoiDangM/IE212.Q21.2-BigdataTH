...
top5_pos_words_by_category = FOREACH (GROUP pos_word_count BY category) {
...
top5_neg_words_by_category = FOREACH (GROUP neg_word_count BY category) {
    ranked = ORDER neg_word_count BY freq DESC, word ASC;
    top5 = LIMIT ranked 5;
    GENERATE FLATTEN(top5) AS (category, word, freq);
};

STORE top5_pos_words_by_category INTO 'output_bai4_top5_positive_words' USING PigStorage(';');
STORE top5_neg_words_by_category INTO 'output_bai4_top5_negative_words' USING PigStorage(';');
